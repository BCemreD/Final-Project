...
const ListingCard: FC<DirectListingV3> = (job) => {
        ...
        <div className=" relative text-white rounded-lg shadow-md w-full max-w-2xl relative">
            ...
            <div className="absolute bg-gray-800 bottom-0 left-0 right-0 opacity-90">
                <div className=" m-4">
                    ...
                    <p className="text-gray-400">{job.asset.description}</p>
                    <div className="flex justify-between mt-2">
                        <div>
                            <p className="text-gray-400">{`for ${job.currencyValuePerToken.displayValue} ${job.currencyValuePerToken.symbol}`}</p>
                            {message !== "" && <p>{message}</p>}
                        </div>
                        <button
